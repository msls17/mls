...
if [ "$(date +%u)" -eq 4 ]; then
for i in 2000;do
echo "账号$s兑换奖品：$(curl -sk -X POST -H "Host: $url" -H "x-token: $token" -H "content-type: application/x-www-form-urlencoded;charset=UTF-8" -d "" "https://$url/activity/icbc/tuesday/v1/receiveEquity.do?type=$i&mobile=$eplus" | jq -r '.msg')"
done
elif [ "$(date +%u)" -eq 5 ]; then
for i in 2006 2007 2002;do
echo "账号$s兑换奖品：$(curl -sk -X POST -H "Host: $url" -H "x-token: $token" -H "content-type: application/x-www-form-urlencoded;charset=UTF-8" -d "" "https://$url/activity/icbc/friday/v1/receiveEquity.do?type=$i&mobile=$eplus" | jq -r '.msg')"
done
else
echo "还没到活动日哦"
fi
