...
if [ "$(date +%u)" -eq 4 ]; then
id=($(curl -sk -X POST -H "Host: $url" -H "x-token: $token" -H "content-type: application/x-www-form-urlencoded;charset=UTF-8" -d "" "https://$url/activity/icbc/tuesday/v1/checkEquity.do" | jq '.body.product[].type'))
