...
token=$(curl -sik -X POST -H "Host: bk011.top" -H "content-type: application/x-www-form-urlencoded; charset=UTF-8" -H "x-requested-with: XMLHttpRequest" -H "user-agent: Mozilla/5.0 (Linux; Android 13; M2102J2SC Build/TKQ1.221114.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/116.0.0.0 Mobile Safari/537.36" -d "email=$user&password=$(echo -n "$pass" | md5sum | awk '{print $1}')" "https://bk011.top/user-login.htm" | grep 'bbs_token=' | grep -v 'bbs_token=deleted' | sed -E 's/.*bbs_token=([^;]+);.*/\1/')
sleep $((10 + RANDOM % 60))
